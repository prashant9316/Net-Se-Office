...
export default function InvoiceLists(props) {
  const [page, setPage] = React.useState(0);
  const [rowsPerPage, setRowsPerPage] = React.useState(10);
  console.log(props.salaryData)
  // Avoid a layout jump when reaching the last page with empty rows.
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - props.salaryData.length) : 0;
...
{/* 
            ...
            <TableBody>
              {(rowsPerPage > 0
                ? props.salaryData.slice(
                    page * rowsPerPage,
                    page * rowsPerPage + rowsPerPage
                  )
                : props.salaryData
              ).map((row) => (
                <TableRow key={row.slipNo}>
                  <TableCell
                    sx={{
                      fontWeight: "500",
                      fontSize: "13px",
